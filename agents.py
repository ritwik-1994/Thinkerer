...
def process_request():
    ...
    image_urls, title, description, script = your_internal_function(username, category)
...
def trigger_function(username, category):
    llm1 = OpenAI(temperature=0.3, openai_api_key = "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", model_name = "text-davinci-003")

    tools = load_tools(["google-serper", "wikipedia", "llm-math"], llm=llm1)
    ...
    internet_generated_ideas = youtube_idea_researcher.run(f"""Find the latest news events in the last 2 months as a continuation from the topic:" + {topic} + ". The information should be original and not a copy of the topic but should resonate it well. Provide an output with an insightful summary of the key information online.""")

    print(internet_generated_ideas)


    llm2 = OpenAI(temperature=0.5, openai_api_key = "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", model_name = "gpt-4")

    tools = load_tools(["google-serper", "wikipedia", "llm-math"], llm=llm2)


    youtube_content_researcher_prompt = f"""As a YouTube Content Researcher AI agent, passionately brainstorm the best content for optimizing engagement on the channel in the {Category} niche. Use all inputs provided to you, to build a video on a single idea . Focus on well-researched content ideas for YouTube engagement optimization. Your final goal is to generate an output optimized for likes/views; with the video Thumbnail Prompt, Video Title, Video Description, Script of the Video"""
...
def extract_info(input_string):
    ...
    try:
        ...
        [Video_Script:A Detailed Scene by Scene Script Which Maximises For Watch Time Of The Video]]
        """)
...
def post_processing(final_dict):
    thumbnail_prompt = content_research_output['Thumbnail Prompt']
    lexica_data = get_lexica_data(thumbnail_prompt)
    for i in range(4):  # Iterate through elements 0, 1, 2, and 3
        lex_data[i] = lexica_data['images'][i]['src']
    content_research_output['Thumbnail'] = lex_data
